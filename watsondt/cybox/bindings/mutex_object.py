...
class MutexObjectType(cybox_common.ObjectPropertiesType):
    ...
    def hasContent_(self):
        if (
            ...
            return False
    def export(self, lwrite, level, namespace_='MutexObj:', name_='MutexObjectType', namespacedef_='', pretty_print=True):
        if pretty_print:
            eol_ = '\n'
        else:
            eol_ = ''
        ...
        self.exportAttributes(lwrite, level, already_processed, namespace_, name_='MutexObjectType')
        if self.hasContent_():
            lwrite('>%s' % (eol_, ))
    ...
    def exportChildren(self, lwrite, level, namespace_='MutexObj:', name_='MutexObjectType', fromsubclass_=False, pretty_print=True):
        ...
        if self.Name is not None:
            self.Name.export(lwrite, level, 'MutexObj:', name_='Name', pretty_print=pretty_print)
    def build(self, node):
        already_processed = set()
...
def parseString(inString):
    ...
    doc = None
#    sys.stdout.write('<?xml version="1.0" ?>\n')
#    rootObj.export(sys.stdout.write, 0, name_="Mutex",
#        namespacedef_='')
    return rootObj
